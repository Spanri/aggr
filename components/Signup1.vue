<template>
  <form class="signup1" @submit.prevent="submit">
    <div class="input-block-wrapper">
      <form-group v-for="(block, index) in blocks" :key="index" class="input-block">
        <p
        ...
        >{{ block.error }}</b-form-invalid-feedback>
        <p v-if="block.model == 'password'" class="input-block__description">
          Пароль должен содержать не менее 8 символов, должны присутствоввать строчные и заглавные
          буквы, должны присутствовать цифры.
        </p>
      </form-group>
    </div>
    <button type="submit" class="button signup1__button">ДАЛЕЕ</button>
  </form>
</template>
